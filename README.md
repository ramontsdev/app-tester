...
### Error while running the app in the iOS Simulator in M1 chips or later:

Run the `yarn run ios --no-build-cache --device` and stop the process after the `./ios` directory is created, then add
the following snipped code in the `./ios/Podfile` in the bottom of the `post_install` block:
...
Then run the `yarn run ios --no-build-cache --device` and select the current running simulator.

Note that this should be done everytime that you run the `expo prebuild`, because the `./ios` directory will be
recreated.
